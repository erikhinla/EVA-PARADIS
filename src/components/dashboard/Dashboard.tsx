...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    async function fetchFunnelData() {
      try {
        ...
        if (data.funnel) setLiveFunnel(data.funnel);
        if (data.leads) setLiveLeads(data.leads);
        setDbConnected(true);
      } catch {
        // Fall back to mock
      }
  ...
  // Resolve funnel metrics: real data if DB connected, mock if not
  const funnel = {
    total_traffic: liveFunnel?.total_visits ?? FUNNEL_DATA.total_traffic,
    of_clicks: liveFunnel?.of_clicks ?? 0,
    email_captures: liveFunnel?.email_captures ?? 0,
    bridge_ctr: liveFunnel?.bridge_ctr ?? FUNNEL_DATA.bridge_ctr,
    net_revenue_today_cents: FUNNEL_DATA.net_revenue_today_cents, // Still mock until OF API
    total_subs: FUNNEL_DATA.total_subs, // Still mock until OF API
    total_leads: liveLeads?.total_leads ?? 0,
    leads_24h: liveLeads?.leads_24h ?? 0,
  };
  ...
  const ctrStatus: "above" | "below" =
    funnel.bridge_ctr >= FUNNEL_DATA.bridge_ctr_benchmark ? "above" : "below";
    ...
    <div className="min-h-screen bg-neutral-950 text-neutral-200 font-sans">
      ...
      <header className="border-b border-neutral-800 bg-neutral-950 px-6 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className={`rounded border ${dbConnected ? "border-emerald-500/40 bg-emerald-500/10 text-emerald-300" : "border-amber-500/40 bg-amber-500/10 text-amber-300"} px-2 py-0.5 text-[10px] font-semibold uppercase tracking-widest`}>
            {dbConnected ? "Live" : "Mock Data"}
          </span>
      ...
      <section className="border-b border-neutral-800 px-6 py-4">
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
          <MetricCard
            label="Total Traffic"
            value={formatNumber(funnel.total_traffic)}
            sub={liveFunnel ? `${funnel.of_clicks} OF clicks` : `${FUNNEL_DATA.traffic_by_layout.A.label}: ${formatNumber(FUNNEL_DATA.traffic_by_layout.A.visits)} / ${FUNNEL_DATA.traffic_by_layout.B.label}: ${formatNumber(FUNNEL_DATA.traffic_by_layout.B.visits)}`}
            status="neutral"
          />
          <MetricCard
            label="Bridge CTR"
            value={formatPercent(funnel.bridge_ctr)}
            sub={`${ctrStatus === "above" ? "\u25B2" : "\u25BC"} vs ${formatPercent(FUNNEL_DATA.bridge_ctr_benchmark)} benchmark`}
            status={ctrStatus}
          />
          <MetricCard
            label="Email Leads"
            value={formatNumber(funnel.total_leads)}
            sub={funnel.leads_24h > 0 ? `+${funnel.leads_24h} today` : "0 today"}
            status={funnel.total_leads > 0 ? "above" : "neutral"}
          />
          <MetricCard
            label="Net Revenue"
            value={formatCurrency(funnel.net_revenue_today_cents)}
            sub="today"
            status="neutral"
          />
          <MetricCard
            label="New Subs"
            value={formatNumber(funnel.total_subs)}
            sub="lifetime"
            status="neutral"
          />
        </div>
